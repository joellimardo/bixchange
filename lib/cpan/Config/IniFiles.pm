...
sub new {
  ...
  #new section for reading strings
  if (defined ($v = delete $parms{'-rawstring'})) {
   $self->{'HARDFILE'} = $v;
...
sub ReadConfig {
  ...
  return undef if (
    ...
    (not defined $self->{cf}) or
     ($self->{cf} eq '')) && (!$self->{'HARDFILE'}) #customized as well for strings
  );
  ...
  # customization limarje for handling string input (no filehandle)
  my $fh; 
  my @lines;
  if (!defined($self->{'HARDFILE'})) {
   $fh = $self->_make_filehandle( $self->{cf} );
  if (!$fh) {
...
=head1 Change log
     ...
     $Log: IniFiles.pm,v $
     Revision 1.1  2007/07/20 12:11:09  joellimardo
     add to SF

     Revision 1.2  2004-10-13 19:25:40  limarje
     allowed for the new -rawstring parameter

     Revision 1.1  2003/11/19 22:57:45  LIMARJE
